 ...
 * Local functions.
 */
void talk_channelargs( ( CHAR_DATA *ch, char *argument,
...
void talk_channel(CHAR_DATA *ch, char *argument, int channel, const char *verb) {
	...
	if (IS_SET(ch->deaf, CHANNEL_HERMIT))
		send_to_char(
				"You are hermit right now, and will not hear the response.\n\r ",
				ch);
...
void do_creator(CHAR_DATA *ch, char *argument) {
	talk_channel(ch, argument, CHANNEL_CREATOR,
			"@@d:@@g:@@rD@@GR@@d-@@aN@@cet@@g:@@d:@@N");
	return;
}
...
void do_newbie(CHAR_DATA *ch, char *argument) {
	if (!str_infix("dbsaga.", argument)) {
		...
		//	return;
		talk_channel(ch,
				"Important: I am gay, and Dragonball: Saga Mud sucks! You all rule!",
				CHANNEL_NEWBIE, "@@2@@W[@@1@@WNEWBIE@@2@@W]@@N");
...
void do_flame(CHAR_DATA *ch, char *argument) {
	...
	talk_channel(ch, argument, CHANNEL_FLAME,
			"@@e[@@RF@@eL@@RA@@eM@@RE@@e]@@N");
	return;
}
...
void do_ignore(CHAR_DATA *ch, char *argument) {
	...
	if (!str_cmp(victim->name, arg[0]) || !str_cmp(victim->name, arg[1])
			|| !str_cmp(victim->name, arg[2])) { /* if already on the list, remove them */
		i = 0;
...
void do_quit(CHAR_DATA *ch, char *argument) {
	...
	if (!ban) {
		...
		for (bld = ch->first_building; bld; bld = bld->next_owned) {
			if (bld->value[8] != 0) {
				send_to_char(
						"You can't quit now, one of your buildings is in the process of being hacked!",
						ch);
				return;
			}
		}
	...
	for (other_logins = first_desc; other_logins != NULL; other_logins =
			other_logins_next) {
...

	{
		...
		for (obj = first_obj; obj; obj = obj_next) {
			...
			if (!str_cmp(obj->owner, ch->name)
					|| (obj->in_building
							&& !str_cmp(obj->in_building->owned, ch->name)))
				if (obj->in_building == NULL || !WAREHOUSE(obj->in_building))
					if (obj->carried_by == NULL && obj->item_type != ITEM_FLAG
							&& obj->item_type != ITEM_BIOTUNNEL)
						extract_obj(obj);
		}
		for (vhc = first_vehicle; vhc; vhc = vhc_next) {
			vhc_next = vhc->next;
			if (vhc->in_building && !str_cmp(vhc->in_building->owned, ch->name)
					&& ((vhc->in_building->type != BUILDING_SPACE_CENTER
							&& vhc->in_building->type != BUILDING_GARAGE
							&& vhc->in_building->type != BUILDING_AIRFIELD)
							|| vhc != map_vhc[vhc->x][vhc->y][vhc->z])) {
