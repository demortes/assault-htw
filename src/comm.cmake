...
void bust_a_prompt( DESCRIPTOR_DATA *d )
{
   ...
   if ( !IS_SET( ch->config, CONFIG_PROMPT ) || (ch->pcdata && ch->pcdata->tutorial > 0))
   {
      sprintf( buf2, "\n\r" );
...
//	    d->connected = CON_READ_MOTD;
	    d->connected = CON_GET_TUTORIAL;
	    write_to_buffer( d, "\n\rWould you like to dedicate a few minutes to learn how to play the game (New/experimental)? \n\r", 0 );
            return;
    ...
    if ( d->connected == CON_GET_TUTORIAL )
    {
        switch ( *argument )
        {
        case 'y': case 'Y':
	    d->character->pcdata->tutorial = -1;
	    d->connected = CON_READ_MOTD;
	    write_to_buffer(d, "Press Enter to begin!\n\r", 0 );
            return;
...
//	ch->fighttimer = 0;
	...
	ch->dead = FALSE;
	ch->fake = FALSE;
	if ( (ch->z == Z_SPACE && !ch->in_vehicle) || ch->z == Z_MEDAL || ch->z == Z_PAINTBALL || ch->z == Z_AIR )
	{
		if ( ch->z == Z_MEDAL )
		{
...
//			ch->pcdata->imcchardata->imcperm = IMCPERM_MORT;
	...
	if ( ch->pcdata->tutorial == -1 )
	{
		ch->pcdata->tutorial = 1;
		move(ch,MAX_MAPS-1,MAX_MAPS-1,ch->z);
	}
	else
	{
		do_look( ch, "auto" );
...
void do_stop( CHAR_DATA *ch, char *argument )
{
	if ( ch->pcdata->tutorial >= 1 )
	{
		ch->pcdata->tutorial = LAST_TUTORIAL;
		return;
	}
	if ( ch->c_sn == -1 )
	{
