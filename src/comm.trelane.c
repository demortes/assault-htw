...
/*        sprintf( buf, "greeting0" );
	...
	do_pipe(dnew->original,"tail -n 100 ../information/connect.txt");
    }
...
void bust_a_prompt( DESCRIPTOR_DATA *d )
{
    ...
    else if ( ch->position == POS_HACKING )
    {
        ...
        sprintf( msg+strlen(msg), "@@N @@r%s's %s (%d)@@N\\>", ch->bvictim->owned, ch->bvictim->name, ch->bvictim->level );
...
//            do_pipe(ch,"tail -n 100 ../information/connect.txt");
    ...
    if ( d->connected == CON_CONFIRM_NEW_PASSWORD )
    {
        ...
        write_to_buffer( d, "\n\rWould you like to play using @@aANSI@@N colors? (Y)es, (N)o, (M)ap colors only: ", 0 );
    ...
    if ( d->connected == CON_GET_ANSI )
    {
        switch ( *argument )
        {
            ...
            case 'n': case 'N':
                write_to_buffer( d, "You may turn colors later on by typing CONFIG COLOR in the game.\n\r\n\r", 0 );
                d->connected = CON_GET_SEX;
                show_smenu_to(d);
                return;
    ...
    if ( d->connected == CON_GET_NEW_MODE )
    {
        ...
        else if ( !str_prefix(argument,"Basic") )
        {
            ...
            {
                write_to_buffer( d,"Sorry, you are only able to go into Basic mode for the first 24 play hours.\n\r", 0 );
                return;
            }
        ...
        switch ( *argument )
        {
            /*        case 'y': case 'Y':
                ...
                case '1':
                    ...
                    write_to_buffer( d, "You will lose EVERYTHING! Type \"yes recreate\" to confirm.\n\r", 0 );
                    return;
            ...
            case '3':
                ...
                {
                    write_to_buffer( d, "None of your buildings were found, please select another option.\n\r", 0 );
                    return;
                }
    ...
    if ( d->connected == CON_GET_NEW_CLASS )
    {
        ...
        for ( cnt = 0;cnt < MAX_CLASS;cnt++ )
            if ((!str_cmp(argument,class_table[cnt].name ) || !str_cmp(argument,class_table[cnt].who_name)) && class_table[cnt].rank <= get_rank(ch) && cnt != CLASS_SCANNER )
        {
            ch->class = cnt;
            if ( ch->played_tot == 0 )
            {
    ...
    if ( d->connected == CON_READ_MOTD )
    {
        ...
        if(i > 0)
        {
            sprintf(buf, "@@d+++ @@WThere are a total of @@d[@@y%d@@d]@@W @@Wchanges made to the MUD today. Type '@@echanges@@W' to view @@d+++@@w\n\r", i);
            send_to_char(buf, ch);
        }
        ...
        if ( map_ch[ch->x][ch->y][ch->z] == NULL || map_ch[ch->x][ch->y][ch->z] != ch )
    ...
    for ( ch = first_char; ch != NULL; ch = ch->next )
    {
            ...
            &&   !str_cmp( d->character->name, ch->name ) )
        {
            ...
            else
            {
                ...
                if ( ch->z == Z_AIR || ch->z == Z_SPACE )   if ( ch->in_vehicle == NULL || !ch->in_vehicle ) ch->z = Z_GROUND;
...
*/
   ...
   sprintf( buf, "@@r+@@G-=-=-=-=-=-=-=-=-=-=- @@c%*s%*s @@G-=-=-=-=-=-=-=-=-=-=-@@r+@@N\n\r", (11 + len) / 2, capitalize(victim->name), (12 - len) / 2, "");
        sprintf( buf+strlen(buf), "@@r[ %s @@r]@@N\n\r",
        IS_GUIDE(victim) ? "NEWBIE HELPER" : IS_NEWBIE( victim ) ? "NEWBIE" : victim->pcdata->who_name );
...
void do_stop( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->c_sn == gsn_paradrop || ch->c_sn == gsn_warp || ch->c_sn == gsn_practice )
    {
        send_to_char( "You can't stop now!\n\r", ch );
        return;
    }
