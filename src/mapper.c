...
@@N'--'   '--' `-'  `--''------' '---'                               \r\n";
	...
	ShowWMap(ch,
			IS_SET(ch->config, CONFIG_SMALLMAP) ? 2 :
			IS_SET(ch->config, CONFIG_TINYMAP) ? 1 : 4, size);
	return;
	...
	for (x = x1; x < x2; x++) {
		for (y = y1; y < y2; y++) {
			...
			if ((bld = map_bld[xx][yy][ch->z]) != NULL)
				sprintf(buf, "%d",
						(bld->type == BUILDING_DUMMY) ?
								bld->value[0] + 100 : 100 + bld->type);
			else
				sprintf(buf, "0%s%d", (sect < 10) ? "0" : "", sect);
			if ((xx != ch->x || yy != ch->y)) {
				for (wch = map_ch[xx][yy][ch->z]; wch; wch =
						wch->next_in_room) {
					if (!can_see(ch, wch))
						continue;
					sprintf(buf + strlen(buf), "P0%d",
					allied(ch,wch) ? 1 : IS_IMMORTAL(wch) ? 2 : 0);
					break;
				}
				if (map_vhc[xx][yy][ch->z]) {
					sprintf(buf + strlen(buf), "V%2d",
							map_vhc[xx][yy][ch->z]->type);
				}
			}
			send_to_char(buf, ch);
		}
...
void ShowBMap(CHAR_DATA *ch, bool quest) {
	...
	for (d = first_desc; d != NULL; d = d->next) {
		...
		if (!can_see(ch, d->character))
			continue;
		if ( paintball(
				ch) && map_bld[d->character->x][d->character->y][1] != NULL && map_bld[d->character->x][d->character->y][1]->type == BUILDING_HQ)
			continue;
		if (sneak(d->character))
			continue;
		if (ch->z != d->character->z)
			continue;
		if (d->character != ch && d->character->x > ch->x - maxx
				&& d->character->x < ch->x + maxx
				&& d->character->y > ch->y - maxx
				&& d->character->y < ch->y + maxx)
			sprintf(p_buf + strlen(p_buf), "%s%s: %s at: %d/%d\n\r",
					(ch->y < d->character->y) ? "North" :
					(ch->y == d->character->y) ?
							(ch->x == d->character->x) ? "Here" : "" : "South",
					(ch->x > d->character->x) ? "West" :
					(ch->x == d->character->x) ? "" : "East",
					d->character->name, d->character->x, d->character->y);
	}
//		if ( ch->z != Z_AIR )
	{
...
//						sprintf( b_buf+strlen(b_buf), "Building: %s, owned by %s, at %d/%d (%s%s)\n\r", bld->name, bld->owned, bld->x, bld->y, (ch->y < bld->y) ? "North" : (ch->y == bld->y) ? "" : "South", (ch->x > bld->x) ? "West" : (ch->x == bld->x) ? "" : "East" );
					...
					if (!bld->owner)
						...
						yours++;
					else if (bld->owner->pcdata->alliance != -1
							&& bld->owner->pcdata->alliance
									== ch->pcdata->alliance)
						allied++;
					else
						enemy++;
	...
	for (y = ch->y; y < MAX_MAPS; y++) {
		if (map_table.type[x][y][ch->z] != last) {
			sprintf(w_buf, "Far north (%d): %s\n\r", y - ch->y,
					wildmap_table[map_table.type[x][y][ch->z]].name);
			break;
		}
	}
	y = ch->y;
	...
	if (ch->in_building && !quest) {
		show_building(ch,
				IS_SET(ch->config, CONFIG_SMALLMAP) ? 2 :
				IS_SET(ch->config, CONFIG_TINYMAP) ? 1 : 4, ch->map);
		return;
	}
...
void show_building(CHAR_DATA *ch, sh_int small, int size) {
	...
	sprintf(exbuf, "%sExits:@@d ",
	IS_SET(ch->config,CONFIG_BLIND) ? "" : "@@r[@@G ");
	if (ch->desc->mxp)
		strcat(exbuf, "\e[1z");
	for (i = 0; i < 4; i++) {
		if (bld->exit[i]) {
	...
	for (i = 0; i < 8; i++) {
		if (bld->resources[i] > 0) {
			...
			sprintf(outbuf + strlen(outbuf), "%d %s\n\r", bld->resources[i],
					(i == 0) ? "Iron" : (i == 1) ? "Skins" :
					(i == 2) ? "Copper" : (i == 3) ? "Gold" :
					(i == 4) ? "Silver" : (i == 5) ? "Rocks" :
					(i == 6) ? "Sticks" : "Logs");
			//			send_to_char( outbuf, ch );
		}
	}
	if (bld->type == BUILDING_SCUD_LAUNCHER
			|| bld->type == BUILDING_NUKE_LAUNCHER
			|| bld->type == BUILDING_ATOM_BOMBER)
		sprintf(borderbuf + strlen(borderbuf),
				"Missile Ready In: %d minutes.\n\r", bld->value[0] / 6);
		...
		sprintf(borderbuf + strlen(borderbuf),
				"Next Interest gain in: %d minutes.\n\r", bld->value[0] / 6);
	else if (bld->type == BUILDING_HACKPORT)
		sprintf(borderbuf + strlen(borderbuf),
				"Backdoor Ready In: %d minutes (%d ticks).\n\r",
				bld->value[0] / 6, bld->value[0]);
	else if (bld->type == BUILDING_SPY_SATELLITE
			|| bld->type == BUILDING_PSYCHIC_TORMENTOR
			|| bld->type == BUILDING_SHOCKWAVE)
		sprintf(borderbuf + strlen(borderbuf),
				"Transmission Ready In: %d minutes.\n\r", bld->value[0] / 6);
	else if (bld->type == BUILDING_SPY_QUARTERS)
		sprintf(borderbuf + strlen(borderbuf),
				"Spy Mission Ready In: %d minutes.\n\r", bld->value[0] / 6);
		...
		sprintf(borderbuf + strlen(borderbuf),
				"Transmitter Ready In: %d minutes.\n\r", bld->value[0] / 6);
	send_to_char(borderbuf, ch);
	send_to_char(outbuf, ch);
...
void do_buildings(CHAR_DATA *ch, char *argument) {
	...
	for (xx = ch->x - maxx; xx <= ch->x + maxx; xx++) {
		for (yy = ch->y - ch->map; yy <= ch->y + ch->map; yy++) {
			...
			if (!all
					&& (str_prefix(argument, bld->owned)
							&& str_cmp(argument, bld->name)))
				continue;
			if (bld->visible == FALSE
					&& (!bld->owner
							|| (!allied(ch, bld->owner) && bld->owner != ch)))
				continue;
...
void do_scanmap(CHAR_DATA *ch, char *argument) {
	...
	while ( TRUE) {
		...
		if (lsect != sect && lsect != -1) {
			sprintf(buf + strlen(buf), "%d %s terrain, ", count,
					wildmap_table[lsect].name);
			count = 0;
		}
		count++;
	}
...
void ShowWMap(CHAR_DATA *ch, sh_int small, int size) {
	...
	int maxx =
			( IS_SET(ch->config, CONFIG_LARGEMAP) && size != 995 && size != 994
					&& size != 998 && size != 997) ? ch->map : ch->map / 2;
	...
	if (!pit && !inMedal && size < 998) /* For quest calls */
	{
		...
		if (!IS_SET(ch->pcdata->pflags, PFLAG_HELPING))
			sprintf(sbuf,
					"  @@cLocation: @@a%3d@@g/@@a%-*d @@g| @@cTerrain: %s%s",
					ch->x, c, ch->y,
					wildmap_table[map_table.type[ch->x][ch->y][ch->z]].color,
					wildmap_table[map_table.type[ch->x][ch->y][ch->z]].name);
		else
			sprintf(sbuf, "@@l(@@g??@@g,@@g??@@l) @@R[@@e%s@@R]@@N",
					planet_table[ch->z].name);
	...
	for (yy = ch->y + maxx; yy >= ch->y - maxx; --yy) { /* every row */
		if (pit && (yy < PIT_BORDER_Y || yy >= MAX_MAPS))
			continue;
//    		safe_strcat( MSL, outbuf, "" );
		strcpy(color, "");
//    		sprintf( color, "" );
		for (xx = ch->x - ch->map; xx <= ch->x + ch->map; ++xx) { /* every column */
			...
			if (ch->x == x && ch->y == y && security == 0) {
				...
				if (inverse) {
					...
					&& (yy < 1 || yy > MEDAL_BORDER_Y || xx < 1
							|| xx > MEDAL_BORDER_X)) {
				sprintf(catbuf, "@@k++@@a");
			} else if (z == Z_AIR
					&& ((map_bld[x][y][Z_GROUND]
							&& map_bld[x][y][Z_GROUND]->type
									== BUILDING_AIRFIELD
							&& map_bld[x][y][Z_GROUND]->active))) {
				sprintf(catbuf, "@@W##@@a");
			} else if (z == Z_AIR
					&& map_table.type[x][y][Z_GROUND] == SECT_MOUNTAIN) {
				sprintf(catbuf, "@@b~/@@a");
			} else if (z == Z_SPACE && map_bld[x][y][Z_GROUND]
					&& map_bld[x][y][Z_GROUND]->type == BUILDING_SPACE_CENTER) {
				sprintf(catbuf, "@@dSP");
			} else if (z == Z_UNDERGROUND && map_bld[x][y][Z_GROUND]
					&& map_bld[x][y][Z_GROUND]->type == BUILDING_TUNNEL) {
				sprintf(catbuf, "@@bTN");
			} else if (pit && (xx < PIT_BORDER_X || xx >= MAX_MAPS)) {
				continue;
			} else if (bld
					&& (bld->visible || (bld->owner && allied(ch, bld->owner))
							|| ch == bld->owner || ch->trust >= 85)) {
				CHAR_DATA *vch;
				char ocolor[MSL];
				...
				if (blind_spot(ch, xx, yy)) {
					...
					char symbol[MSL];
					bool mxp = TRUE;
					def = ((!ch->security && bld->owner == ch
							&& defense_building(bld)));
					...
					if (security == 0) {
						sprintf(ocolor, "%s",
								...
								(bld->visible == FALSE) ? "@@W" :
								(bld->hp < bld->maxhp
										&& bld->hp > bld->maxhp / 2) ? "@@b" :
								(bld->hp < bld->maxhp / 2) ? "@@Q" : "@@G");
						if (ch == bld->owner || bld->protection > 0
								|| ((bld->type == BUILDING_DUMMY
						...
						sprintf(ocolor, "%s",
								(build_table[bld->type].act != BUILDING_DEFENSE) ?
										"@@d" :
								bld->value[6] <= 3 ? "@@g" :
								bld->value[6] <= 6 ? "@@b" :
								bld->value[6] <= 9 ? "@@R" : "@@e");
					...
					if (security == 2
							&& build_table[bld->type].act == BUILDING_DEFENSE
							&& bld->owner && bld->owner == ch) {
						sprintf(symbol, "%*d", small, bld->value[6]);
					} else {
						if (bld->type != BUILDING_DUMMY)
							sprintf(symbol, "%s",
									build_table[bld->type].symbol);
						else
							sprintf(symbol, "%s",
									build_table[bld->value[0]].symbol);
					}
					if (mxp) {
						if (ch->desc->mxp)
							strcat(catbuf, "\e[1z");
			...
			} else if ((map_ch[x][y][z] != NULL
					|| IS_SET(ch->effect, EFFECT_VISION)) && size != 998
					&& size != 997) {
				CHAR_DATA *wch;
				bool vehicle = FALSE;
				bool allied = FALSE;
				...
				if (blind_spot(ch, xx, yy)) {
					...
					if (ppl > 9)
						ppl = 9;
					if ( IS_SET(ch->effect, EFFECT_VISION) && ppl == 0
							&& number_percent() < 5)
						ppl++;
					...
					if (ppl <= 0) {
						sprintf(color, "%s",
								(!str_cmp(
										wildmap_table[map_table.type[x][y][ch->z]].color,
										color)) ?
										"" :
										wildmap_table[map_table.type[x][y][ch->z]].color);
						if (inverse)
							strcat(color, "@@i");
						sprintf(catbuf, "%s%s", color,
								wildmap_table[map_table.type[xx][yy][ch->z]].mark);
					} else {
						if (vehicle) {
							if (small == 1)
								sprintf(catbuf, "%s%s@@N",
										imm ? "@@y" : newbie ? "@@a" :
										allied ? "@@r" : "@@e", ppl_c);
							if (small == 2)
								sprintf(catbuf, "%s[%s@@N",
				...
				sprintf(catbuf + strlen(catbuf), "%s",
						(blind_spot(ch, xx, yy)) ?
								"  " :
								wildmap_table[map_table.type[x][y][ch->z]].mark);
				if (terrain[map_table.type[x][y][ch->z]] == FALSE)
					terrain[map_table.type[x][y][ch->z]] = TRUE;
	...
	if (my_get_hours(ch, TRUE) < 2 && !IS_SET(ch->config, CONFIG_NOLEGEND)) {
		...
		for (j = 0; j < SECT_MAX; j++) {
			if (!terrain[j])
				continue;
			sprintf(tbuf + strlen(tbuf), "%s%s %-11s ", wildmap_table[j].color,
					wildmap_table[j].mark, wildmap_table[j].name);
			l++;
			if (l % 4 == 0)
...
void ShowSpace(CHAR_DATA *ch) {
	...
	sprintf(buf + strlen(buf), "%s  --/  SCN: %-3d    WPN: %-3d  \\-- \n\r",
			pref, j, vhc ? vhc->range : 0);
	sprintf(buf + strlen(buf),
			"%s / ______________      @@RSS@@dAA@@aFF@@c_  \\ \n\r", pref);
	...
	while (1) {
		...
		if (bld != NULL) {
			sprintf(buf, "%s owned by %s: %d.\n\r",
					bld->type == BUILDING_DUMMY ?
							build_table[bld->value[0]].name : bld->name,
					bld->owned,
					(dir == DIR_NORTH) || (dir == DIR_SOUTH) ? bld->y : bld->x);
			send_to_char(buf, ch);
		}
	}
	return;
